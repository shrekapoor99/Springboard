...
the member name. */
SELECT DISTINCT CONCAT(firstname, ' ', surname) AS member, name AS facility FROM Members
...
SELECT Facilities.name AS facility, 
       Members.memid AS member_id,
       Members.firstname || ' ' || Members.surname AS member_name, 
       COUNT(Bookings.bookid) AS usage_count
FROM Facilities
JOIN Bookings ON Facilities.facid = Bookings.facid
JOIN Members ON Bookings.memid = Members.memid
WHERE Members.memid != 0
GROUP BY Facilities.name, Members.memid, Members.firstname, Members.surname
ORDER BY Facilities.name, Members.firstname, Members.surname;
